...
from model_europarl import LM, repackage_hidden
...
parser.add_argument("--dp_prob", type=float, default=0.3,
...
parser.add_argument('--save', type=str,  default='lm_model_europarl',
...
def train():
    ...
    if args.bptt:
        train_data_it= europarl_dataset("europarl_monolingual_bptt")
        valid_data_it = europarl_dataset("europarl_monolingual_valid_bptt")
        vocab_size = len(train_data_it.dataset.fields["text"].vocab.itos)
    else:
        train_data_it=europarl_translation_dataset("europarl_monolingual")
        valid_data_it=europarl_translation_dataset("europarl_monolingual_valid")
        vocab_size = len(train_data_it.dataset.fields["src"].vocab.itos)



    criterion = nn.CrossEntropyLoss()

    model = LM(embed_size=args.embed_size,vocab_size=vocab_size, layers=args.num_layers, dropout=1 - args.dp_prob,
               type=args.rnn_type, hidden_size=args.hidden_size).to(device)

    if args.optimizer == "SGD":
        optimizer = optim.SGD(model.parameters(), lr = args.initial_lr)
    else:
        optimizer = optim.Adam(model.parameters(), lr = args.initial_lr, amsgrad=True) # only translation dataset
    model.to(device)
    ...
    for epoch in range(args.num_epochs):
        train_p = run_epoch(model, optimizer, train_data_it, criterion, True, lr = lr)
        valid_p = run_epoch(model, optimizer, valid_data_it, criterion)
        if valid_p < best_perplexity:
            ...
            with open(args.save + "best_perplexity_checkpoint_" + args.rnn_type+ ".pth", "wb") as f:
                torch.save(checkpoint, f)
            with open(args.save + "best_perplexity_model_"+ args.rnn_type+ ".pth", "wb") as f:
                torch.save(model, f)
        scheduler.step()
