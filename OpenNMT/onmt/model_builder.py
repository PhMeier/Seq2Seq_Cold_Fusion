...
def build_base_model(model_opt, fields, gpu, checkpoint=None, gpu_id=None):
    ...
    if not model_opt.copy_attn:
        ...
        if model_opt.decoder_type == "cold_fusion": # added cold fusion generator

            if model_opt.use_gating_for_logit_computation:
                ...
                generator = nn.Sequential(
                    nn.Linear(model_opt.dec_rnn_size * 2,
                          len(fields["tgt"].base_field.vocab)),
                    nn.ReLU(),
                    Cast(torch.float32),
                    gen_func
                )
        elif model_opt.decoder_type == "dynamic_fusion":  # added dynamic fusion generator
            generator = nn.Sequential(
                nn.Linear(model_opt.dec_rnn_size*2,
                          len(fields["tgt"].base_field.vocab)),
                Cast(torch.float32),
