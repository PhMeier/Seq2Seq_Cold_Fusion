...
class LM(nn.Module):
    ...
    def __init__(self, layers, type, hidden_size, embed_size, vocab_size, dropout):
    ...
    def forward(self, input: Tensor, hidden):
        #input size: num_steps, batch_size
        ...
        return output.view(input.size(0), input.size(1), self.vocab_size), hidden
    ...
    def init_hidden(self, batch_size):
        ...
        if self.type == "LSTM":
            return (Variable(weight.new(self.layers, batch_size, self.hidden_size).zero_()).to(device),
                    Variable(weight.new(self.layers, batch_size, self.hidden_size).zero_()).to(device))
        else:
            return Variable(weight.new(self.layers, batch_size, self.hidden_size).zero_().to(device))
